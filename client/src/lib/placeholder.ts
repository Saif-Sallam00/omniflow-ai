...
// Local, bundled image fallback — an inline SVG data URI, so a broken/missing
// image never triggers an external round-trip (previously hit placehold.co).
...
// onError handler: swap to the local fallback and detach itself to avoid loops.
